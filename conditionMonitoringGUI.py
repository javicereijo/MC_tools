...
class Main(QtGui.QDialog):
  ...
  def initUI(self):
    ...
    self.setWindowTitle("Condition Monitoring GUI")
  ...
  def onChanges(self,pvname=None, value=None, char_value=None, **kw):
    self.dataPerSample=value.size;
    if self.bufferIndex>=self.bufferSize:
      self.buffer.fill(0)
      self.bufferIndex=0
    self.buffer[self.bufferIndex:self.bufferIndex+value.size]=value;
    self.bufferIndex=self.bufferIndex+value.size
    if self.bufferIndex>=self.bufferSize:
      print "Buffer Full"
  ...
  def doFFT(self,data):
    ...
    y = data*self.countToGScaleFactor
